  ...
  if(newMsgList && chatThat) {
    for (var j in newMsgList) {
      ...
      if (chatThat && newMsg.getSession().id() == friendId) {
        ...
        if (newMsg.elems[0].type != 'TIMCustomElem') {
          chatThat.addMessage(newMsg.elems[0].content.text, false,"text", chatThat)
        }else{
        chatThat.addMessageFriend(newMsg.elems[0], false,"custom", chatThat)
        }
      }
    }
  }
...
//发送消息(自定义消息)
function sendCustomMsg(msgs, cbOk){
  var data = msgs.Data; //数据
  var desc = msgs.Desc; //描述
  var ext = msgs.Ext; //拓展字段
  var msgLen = im.Tool.getStrBytes(data);
  if (msgs.length < 1) {
    alert("发送的消息不能为空!");
    return;
  }
  var maxLen, errInfo;
  if (selType == im.SESSION_TYPE.C2C) {
    maxLen = im.MSG_MAX_LENGTH.C2C;
    errInfo = "消息长度超出限制(最多" + Math.round(maxLen / 3) + "汉字)";
  } else {
    maxLen = im.MSG_MAX_LENGTH.GROUP;
    errInfo = "消息长度超出限制(最多" + Math.round(maxLen / 3) + "汉字)";
  }
  if (msgLen > maxLen) {
    alert(errInfo);
  ...
  var msg = new im.Msg(selSess, true, -1, -1, -1, friendId, 0, friendName);
  var custom_obj = new im.Msg.Elem.Custom(data, desc, ext);
